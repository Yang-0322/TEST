...
def make_CLIMB_dataloader(cfg, all_iters=False):
    ...
    # 使用配置文件中的 CLUSTER_BATCH_SIZE，如果不存在则默认使用 1024
    cluster_batch_size = getattr(cfg.DATALOADER, 'CLUSTER_BATCH_SIZE', 1024)
    cluster_loader = DataLoader(
        cluster_set, batch_size=cluster_batch_size, shuffle=False, num_workers=num_workers
    )
