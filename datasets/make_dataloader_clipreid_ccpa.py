...
from .bases import ImageDataset, AttributeImageDataset
...
def train_collate_fn(batch):
    ...
    """
    imgs, pids, camids, viewids , _, attrs = zip(*batch)
    pids = torch.tensor(pids, dtype=torch.int64)
    viewids = torch.tensor(viewids, dtype=torch.int64)
    camids = torch.tensor(camids, dtype=torch.int64)
    attrs = torch.stack(attrs, dim=0)
    return torch.stack(imgs, dim=0), pids, camids, viewids, attrs
...
def make_dataloader(cfg):
    ...
    train_set = AttributeImageDataset(dataset.train, cfg.DATASETS.ATTR_PATH, dataset.pid2label, train_transforms)
    train_set_normal = AttributeImageDataset(dataset.train, cfg.DATASETS.ATTR_PATH, dataset.pid2label, val_transforms)
