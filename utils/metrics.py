...
def eval_func(distmat, q_pids, g_pids, q_camids, g_camids, max_rank=50):
    ...
    for q_idx in range(num_q):
        ...
        # 对qiuxiu数据集适配
        if (num_q == 776 and num_g == 4232) or (num_q == 839 and num_g == 4858):
            remove = np.zeros_like(g_pids[order], dtype=bool)
        else:
...
class R1_mAP_eval():
    ...
    def compute(self):  # called after each epoch
        ...
        if self.reranking:
            print('=> Enter CAJ reranking')
            # distmat = re_ranking(qf, gf, k1=50, k2=15, lambda_value=0.3)
            distmat_all = compute_jaccard_distance(
                            features=feats,
                            cam_labels=np.asarray(self.camids),
                            epoch='test'
                        )
            Nq = qf.size(0)
            Ng = gf.size(0)
            distmat1 = distmat_all[:Nq, Nq:]
            distmat2 = euclidean_distance(qf, gf)
